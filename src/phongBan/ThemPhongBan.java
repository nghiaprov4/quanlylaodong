...
public class ThemPhongBan extends JFrame implements ActionListener{
	...
	private JButton btnThem, btnXoaTrang, btnTimKiem;
	...
	public ThemPhongBan() throws SQLException {
		...
		pnNorth = new JPanel();
		pnNorth.add(lblTitle = new JLabel("THÊM PHÒNG BAN"));
		...
		LoadDatabase();
		lblSoLuong.setText("Số lượng phòng ban: " +list.getSize());	
...
//		if(!(tenPB.length() > 0 && tenPB.matches("^([A-Z][a-z0-9]*\\s*)*[A-Z][a-z0-9]*$"))){
//			JOptionPane.showMessageDialog(this, "Lỗi tên phòng ban.");
//			return false;
//		}
		if(!(tenPB.length() > 0 )){
			JOptionPane.showMessageDialog(this, "Bạn phải nhập tên phòng ban.");
			return false;
		}
		return true;
	}
