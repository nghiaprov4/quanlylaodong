...
public class SuaPhongBan extends JFrame implements ActionListener{
	...
	public SuaPhongBan() throws SQLException {
		...
		pnNorth = new JPanel();
		pnNorth.add(lblTitle = new JLabel("SỬA PHÒNG BAN"));
		...
		pnTask.add(btnSua = new JButton("Sửa"));
		...
		btnSua.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				int ret=JOptionPane.showConfirmDialog(null,
						"Bạn muốn sửa phòng ban này?", "Sửa",
				...
				try {
					...
					if(list.editElement(maPB, pb) == true) {
						JOptionPane.showMessageDialog(null, "Sửa thành công");
						LoadDatabase();
						tbPhongBan.setRowSelectionInterval(rowClicked, rowClicked);
					}else {
						JOptionPane.showMessageDialog(null, "Không được sửa mã Phòng Ban");
						txtmaPB.selectAll();
	...
	public boolean validData() {
		...
		if(maPB.length() > 0){
			try {
				int x = Integer.parseInt(maPB);
				if(x <= 0) {
					JOptionPane.showMessageDialog(null, "Mã phòng ban phải nhập số dương.");
					return false;
				}
			}catch (Exception e) {
					// TODO: handle exception
				JOptionPane.showMessageDialog(null, "Mã phòng ban là một số.");
				return false;
			}
		}
		
		if(maPB.length() == 0) {
			JOptionPane.showMessageDialog(null, "Bạn phải nhập mã phòng ban.");
			return false;
		}
		
//		if(!(tenPB.length() > 0 && tenPB.matches("^([a-zA-Z0-9]\\s*)*[a-zA-Z0-9]$"))){
//			JOptionPane.showMessageDialog(this, "Lỗi tên phòng ban.");
//			return false;
//		}
		if(!(tenPB.length() > 0 )){
			JOptionPane.showMessageDialog(this, "Bạn phải nhập phòng ban.");
			return false;
		}
		return true;
	}
