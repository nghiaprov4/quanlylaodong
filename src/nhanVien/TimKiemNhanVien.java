...
public class TimKiemNhanVien extends JFrame implements ActionListener{
	...
	public TimKiemNhanVien() throws SQLException {
		...
		pnNorth = new JPanel();
		pnNorth.add(lblTitle = new JLabel("TÌM KIẾM NHÂN VIÊN"));
...
//				cbbPhongBan.setSelectedItem(tbNhanVienModel.getValueAt(rowClicked, 5).toString());
	...
	public void Search() {
		...
		if(str != null && str.trim().length() > 0) {
			try {
				...
				NhanVien nhanVien = list.search(id);
				if(nhanVien != null) {
					tbNhanVienModel.setRowCount(0);
